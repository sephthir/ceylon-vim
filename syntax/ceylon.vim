...
"  unlet b:current_syntax
  ...
  syn match  ceylonDocTags  contained "@\(see\|param\|exception\|throws\|since\)\s\+\S\+" contains=ceylonDocParam
  syn match  ceylonDocParam contained "\s\S\+"
  syn match  ceylonDocTags  contained "@\(version\|author\|return\|deprecated\|serial\|serialField\|serialData\)\>"
  syntax case match
endif
...
if exists("ceylon_highlight_functions")
  if ceylon_highlight_functions == "indent"
    ...
    syn region ceylonFuncDef start=+^\s\+\(\(shared\|actual\|formal\|default\|abstract\)\s\+\)*\(\(void\|function\|\([A-Za-z_][A-Za-z0-9_$]*\.\)*[A-Z][A-Za-z0-9_$]*\)\(<[^>]*>\)\=\(\[\]\)*\s\+[a-z][A-Za-z0-9_$]*\|[A-Z][A-Za-z0-9_$]*\)\s*([^0-9]+ end=+)+ contains=ceylonScopeDecl,ceylonType,ceylonStorageClass,ceylonComment,ceylonLineComment,@ceylonClasses
  endif
...
if exists("ceylon_highlight_debug")
  ...
  syn match ceylonDebug "\<System\.\(out\|err\)\.print\(ln\)*\s*("me=e-1 contains=ceylonDebug.* nextgroup=ceylonDebugParen
